...
local input_directory = deathvox.ModPath .. "data/weapon_stats/"
...
for _,filename in pairs(file_util:GetFiles(weapon_stats_directory)) do
	...
	if extension == ".csv" then 
		...
		for raw_line in input_file:lines() do 
			line_num = line_num + 1
			local raw_csv_values = string.split(raw_line,",") --my favorite type of tea is chai tea
			
			local weapon_id = raw_csv_values[STAT_INDICES.id]
			if weapon_id and utf8.to_upper(weapon_id) ~= "NULL" then 
				...
				if wtd then 
					...
					if can_shoot_through_enemy == "null" then 
						wtd.can_shoot_through_enemy = nil
					elseif can_shoot_through_enemy ~= nil then 
						wtd.can_shoot_through_enemy = can_shoot_through_enemy
					end
					if can_shoot_through_shield == "null" then 
						wtd.can_shoot_through_shield = nil
					elseif can_shoot_through_shield ~= nil then 
						wtd.can_shoot_through_shield = can_shoot_through_shield
					end
					if can_shoot_through_wall == "null" then 
						wtd.can_shoot_through_wall = nil
					elseif can_shoot_through_wall ~= nil then 
						wtd.can_shoot_through_wall = can_shoot_through_wall
					end
					if armor_piercing_chance == "null" then 
						wtd.armor_piercing_chance = nil
					elseif armor_piercing_chance ~= nil then 
